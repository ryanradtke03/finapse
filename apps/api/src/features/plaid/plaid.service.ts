...
export async function exchangePublicToken({
  ...
  console.log(
    "[exchange] metadata accounts",
    institution.accounts.map((a) => ({ id: a.id, name: a.name, mask: a.mask })),
  );
  ...
  console.log("[exchange] existingItem lookup", {
    found: !!existingItem,
  ...
  if (existingItem) {
    ...
    console.log(
      "[exchange] stored accounts in DB",
      storedAccounts.map((a) => ({ id: a.plaidAccountId, name: a.name })),
    );

    const storedIds = new Set(storedAccounts.map((a) => a.plaidAccountId));
    const newAccounts = institution.accounts.filter(
      (a) => !storedIds.has(a.id),
    );

    console.log(
      "[exchange] new accounts from metadata (not yet in DB)",
      newAccounts.map((a) => ({ id: a.id, name: a.name })),
    );
    ...
    if (newAccounts.length > 0) {
      ...
      let accountsRes;
      try {
        accountsRes = await plaidClient.accountsGet({
          access_token: decryptedToken,
        });
        console.log(
          "[exchange] accountsGet returned",
          accountsRes.data.accounts.length,
          "accounts",
        );
        console.log(
          "[exchange] accountsGet detail",
  ...
  try {
    ...
    console.log("[exchange] token exchanged ok", {
  ...
  try {
    ...
    console.log(
      "[exchange] accountsGet for new item returned",
      accountsRes.data.accounts.length,
      "accounts",
    );
    console.log(
...
export async function syncTransactions(userId: string, plaidItemId: string) {
  ...
  if (item.status === "DISCONNECTED") {
    ...
    throw Object.assign(new Error("PlaidItem is disconnected"), {
      status: 400,
    });
  }

  console.log("[sync] item found", {
    itemId: item.id,
    institutionId: item.institutionId,
    status: item.status,
    hasCursor: !!item.transactionCursor,
  });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    console.log(
      "[sync] step 1 — upserting",
      allAccounts.length,
      "accounts from accountsGet",
    );
    for (const acc of allAccounts) {
      console.log("[sync] upserting account", {
    ...
    for (const t of added) {
      ...
      if (!account) {
        console.warn("[sync] no account found for transaction", {
    ...
    for (const t of modified) {
      ...
      if (!account) {
        console.warn("[sync] no account found for modified transaction", {
          transaction_id: t.transaction_id,
          account_id: t.account_id,
        });
        continue;
...
export async function deletePlaidItem(
  ...
  if (!item) {
    const err = Object.assign(new Error("Plaid item not found"), {
      status: 404,
    });
    throw err;
  }
  ...
  try {
    ...
    console.warn(
      `[deleteItem] Plaid itemRemove failed for item ${itemId}, proceeding with DB delete:`,
      plaidErr,
    );
  }
...
export async function deletePlaidAccount(
  userId: string,
  accountId: string,
): Promise<void> {
