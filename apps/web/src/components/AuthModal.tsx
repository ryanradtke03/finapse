  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        ...
        <p className="text-brand-text-secondary text-left text-sm">Log in to your Finapse account</p>
      </div>
      <div className="mt-4">
        <div>
      ...
      <div className="flex items-center w-full justify-center mt-6">
        <span className="text-xs text-brand-text-secondary">
          ...
          <button type="button" onClick={onSwitch} className="text-brand-green cursor-pointer">Sign up free</button>
        </span>
      </div>
    </form>
  );
...
function SignupForm({ onSwitch }: { onSwitch: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        <h3 className="text-brand-text font-semibold text-lg">Create your account</h3>
        <p className="text-brand-text-secondary text-left text-sm">Free to start — no credit card needed</p>
      </div>
      <div className="mt-4">
        <div>
      ...
      <div className="flex items-center w-full justify-center mt-6">
        <span className="text-xs text-brand-text-secondary">
          ...
          <button type="button" onClick={onSwitch} className="text-brand-green cursor-pointer">Log in</button>
        </span>
      </div>
    </form>
  );
...
export function AuthModal({ open, onClose, defaultTab = "login" }: { open: boolean; onClose: () => void; defaultTab?: "login" | "signup" }) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-6 w-8 h-8 bg-brand-surface-input rounded-xl text-brand-text-secondary border border-brand-border-subtle cursor-pointer transition-all duration-200 hover:text-brand-text hover:border-brand-text">
          ✕
        </button>
